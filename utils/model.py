...
def get_vocoder(config, device):
    ...
    if name == "MelGAN":
        ...
        config_file = '/data/conggaoxiang/vocoder/hifi-gan-master/My16_test1_hifigan/config.json'
        with open(config_file) as f:
            data = f.read()
        json_config = json.loads(data)
        h = AttrDict(json_config)
        ...
        vocoder = Generator(h).to(device)
        state_dict_g = load_checkpoint('/data/conggaoxiang/vocoder/hifi-gan-master/My16_test1_hifigan/g_01880000',
                                       device)
        vocoder.load_state_dict(state_dict_g['generator'])
        vocoder.eval()
        vocoder.remove_weight_norm()
        ...
        config_file = '/data/conggaoxiang/vocoder/hifi-gan-master/My16_test1_hifigan_V2C/config.json'
        with open(config_file) as f:
            data = f.read()
        json_config = json.loads(data)
        h = AttrDict(json_config)
        ...
        vocoder = Generator(h).to(device)
        state_dict_g = load_checkpoint('/data/conggaoxiang/vocoder/hifi-gan-master/My16_test1_hifigan_V2C/g_00150000',
                                       device)
        vocoder.load_state_dict(state_dict_g['generator'])
        vocoder.eval()
        vocoder.remove_weight_norm()
